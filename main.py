...
def explore_data(df):
    ...
    print("\n\nCorrelation Matrix:")
    print(df.corr()[['Concrete Compressive Strength']])

    df_corr = df.corr()[['Concrete Compressive Strength']]
    most_correlated_feature = df_corr.drop(
        'Concrete Compressive Strength').idxmax().values[0]

    # TODO: Find the feature name with highest correlation to target
...
def preprocess_data(df, test_size=0.2, random_state=42):
    ...
    X = df.drop('Concrete Compressive Strength',
                axis=1)  # All columns except target
